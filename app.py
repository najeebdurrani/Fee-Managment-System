...
def all_session():
    ...
    if request.method == "POST":
        ...
        # Get all records again
        cursor.execute(
            "SELECT sessions.title, COUNT(students.roll_no), sum(fee.total_fee), sum(fee.sub_fee), sum(fee.rem_fee) FROM students INNER JOIN fee on students.roll_no = fee.roll_no RIGHT JOIN sessions ON students.session = sessions.title GROUP BY sessions.title")
        sessions = cursor.fetchall()
...
def search():
    ...
    if request.method == "POST":
        ...
        session = request.form.get("session")
        cursor.execute(
            "SELECT * from students INNER JOIN fee ON students.roll_no = fee.roll_no where students.session = %s",
            (session,))
        students = cursor.fetchall()
...
def update(roll_no):
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT * from students INNER JOIN fee ON students.roll_no = fee.roll_no WHERE students.roll_no = %s",
            (roll_no,))
...
def delete(roll_no):
    ...
    stud = cursor.fetchone()
    if stud is None:
        return "No record found by Roll No = " + str(
            roll_no) + ". Kindly go back to <a href='/view'> View All Students </a>"
    else:
